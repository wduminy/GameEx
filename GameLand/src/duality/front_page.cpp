...
const int FONT_HEIGHT = 34;
	...
	FrontPage::FrontPage() : _font(),
			_panel(new Panel(-0.8,0.8,PANEL_WIDTH,PANEL_HEIGHT,PANEL_PIX_W,PANEL_PIX_H)), _escaped(false) {
		add_part(_panel);
		set_active(true);
	}
	...
	void FrontPage::initialise(const ResourceContext &rc, const DrawContext& dc) {
		...
		_font->render_center(*_panel,PANEL_CENTER,at+=FONT_HEIGHT,"<- and -> rotates the view");
		_font->render_center(*_panel,PANEL_CENTER,at+=FONT_HEIGHT,"Esc to go away");
		_font->render_center(*_panel,PANEL_CENTER,at+=FONT_HEIGHT,"Enter to play");
